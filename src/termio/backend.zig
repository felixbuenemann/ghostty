...
pub const Kind = enum { exec, external_io };
...
pub const Config = union(Kind) {
    ...
    external_io: termio.ExternalIO.Config,
};
...
pub const Backend = union(Kind) {
    exec: termio.Exec,
    external_io: termio.ExternalIO,

    pub fn deinit(self: *Backend) void {
        switch (self.*) {
            .exec => |*exec| exec.deinit(),
            .external_io => |*v| v.deinit(),
        }
    }
    ...
    pub fn initTerminal(self: *Backend, t: *terminal.Terminal) void {
        switch (self.*) {
            ...
            .external_io => |*v| v.initTerminal(t),
        }
    }
    ...
    pub fn threadEnter(
        ...
        switch (self.*) {
            ...
            .external_io => |*v| try v.threadEnter(alloc, io, td),
        }
    }
    ...
    pub fn focusGained(
        ...
        switch (self.*) {
            ...
            .external_io => |*v| try v.focusGained(td, focused),
        }
    }
    ...
    pub fn resize(
        ...
        switch (self.*) {
            ...
            .external_io => |*v| try v.resize(grid_size, screen_size),
        }
    }
    ...
    pub fn queueWrite(
        ...
        switch (self.*) {
            ...
            .external_io => |*v| try v.queueWrite(alloc, td, data, linefeed),
        }
    }
    ...
    pub fn childExitedAbnormally(
        ...
        switch (self.*) {
            ...
            .external_io => |*v| try v.childExitedAbnormally(
                gpa,
                t,
                exit_code,
                runtime_ms,
    ...
    pub fn getProcessInfo(self: *Backend, comptime info: ProcessInfo) ?ProcessInfo.Type(info) {
        return switch (self.*) {
            ...
            .external_io => |*v| v.getProcessInfo(info),
        };
    }
};
...
pub const ThreadData = union(Kind) {
    ...
    external_io: termio.ExternalIO.ThreadData,

    pub fn deinit(self: *ThreadData, alloc: Allocator) void {
        switch (self.*) {
            .exec => |*exec| exec.deinit(alloc),
            .external_io => |*v| v.deinit(alloc),
        }
    }
